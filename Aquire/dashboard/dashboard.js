  ...
  store :MongoStore.create({ mongoUrl: jsonconfig.mongodb_url})
    ...
    const renderTemplate = (res, req, template, data = {}) => {
      ...
      const baseData = {
        ...
        url: res,
        req: req,
        image: `https://cdn.discordapp.com/avatars/818385882641858612/209df0ed593b120298be0a1375bcaa11.webp?size=512`,
      ...
      try {
        ...
        if (req.session.backURL) {
          ...
          if (member) {
            ...
            loginLogs.send({
              username: 'Login Logs',
              avatarURL: `https://cdn.discordapp.com/avatars/818385882641858612/209df0ed593b120298be0a1375bcaa11.webp?size=512`,
              embeds: [login]
            });
          }
...
var url = req.protocol + '://' + req.get('host') + req.originalUrl;
    ...
    app.get("/support", (req, res) => {
      res.redirect(`https://discord.gg/tZARmUsnZU`)
    });
...
member.send(new MessageEmbed().setColor('GREEN').setFooter(`Powered by https://syfer.aquire.tk`).setTitle(`Application #${ticketID}`).setDescription(`Hey ${member.user.username}! Your form was Submitted and ready to be judged.\n\n**Form ID**: \`${ticketID}\`\n**Time:** ${moment(new Date()).format("dddd, MMMM Do YYYY HH:mm:ss")}`)).catch(()=>{});
...
channel.send(embed)
 ...
 app.get("/redeem", checkAuth, (req, res) => {
    ...
    app.get("/dashboard", checkAuth, (req, res) => {
      ...
      const server = client.guilds.cache.get('867388904869068811');
      ...
      if (Object.prototype.hasOwnProperty.call(data, "sendEmbed")) {
        ...
        if (ticketSettings.ticketToggle == "true") {
          ...
          if (ticketSettings.ticketType == "reaction" && ticketSettings.ticketCustom == "false") {
            ...
            if (storedSettings.isPremium == "false") {
              ticketSettings.ticketFooter = "Powered by syfer.aquire.tk";
            } else {
            ...
            let reactionFooter = "Powered by syfer.aquire.tk";



            let footer = "Powered by syfer.aquire.tk";
            ...
            if (storedSettings.isPremium == "false") {
              ticketEmbed.setFooter(`Powered by syfer.aquire.tk`)
            } else {
...
if(maintenance && maintenance.toggle == "true") {
    ...
    app.post('/dblwebhook', webhook.middleware(), async (req) => {
      ...
      const msg = new Discord.MessageEmbed()
        .setAuthor('Voting System', `https://cdn.discordapp.com/avatars/818385882641858612/209df0ed593b120298be0a1375bcaa11.webp?size=512`)
        .setColor('#7289DA')
        .setTitle(`${apiUser.username} Just Voted`)
        .setDescription(`Thank you **${apiUser.username}#${apiUser.discriminator}** (${apiUser.id}) for voting **Syfer**!`)
      Hook.send(msg);
...
if(config.datadogApiKey){
  metrics.init({ apiKey: jsonconfig.datadogApiKey, host: 'Syfer', prefix: 'Syfer.' });
