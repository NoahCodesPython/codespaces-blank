...
logging2.moderation.mute_role = muteRole.id
    ...
    if(textChats){
      ...
      .setColor('PURPLE')
      .setDescription(`Hey Users! I'm **Aquire**.\n\nThank you for inviting me to your server as it means a lot to us! You can get started with [\`++help\`](https://aquire.tk) & customise your server settings by accessing the Dashboard [\`here\`](https://aquire.tk/dashboard/${guild.id}).\n\n__**Current News**__\n\`\`\`\nWe are currently giving premium to all servers until 1000 guilds! If interested Please visit https://aquire.tk/redeem\`\`\`\n\nAgain, thank you for inviting me! (this server is now very pog)\n**- Aquire**`)
      .addField(
        '\u200b', 
        '**[Invite](https://aquire.tk/invite) | ' +
        '[Support Server](https:/syfer.aquire.tk/support) | ' +
        '[Dashboard](https://aquire.tk/dashboard)**'
      );
    ...
    .setColor(`BLUE`)
    .setTitle('New Server')
    .setThumbnail(`https://aquire.tk/logo`)
    .setDescription(`Syfer was added to a new Server!`)
    .addField(`Server Name`, `\`${guild.name}\``, true)
    .addField(`Server ID`, `\`${guild.id}\``, true)
    .setFooter(`${this.client.guilds.cache.size} guilds `,  'https://aquire.tk/logo.png');

welcomeClient.send({
   username: 'Aquire',
        avatarURL: 'https://aquire.tk/logo.png',
        embeds: [welcomeEmbed],
})
