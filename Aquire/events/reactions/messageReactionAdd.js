...
let arraylength = array.length
      ...
      if(arraylength > ticketlimit || arraylength == ticketlimit) {
        ...
        message.channel.send(new discord.MessageEmbed().setColor(message.client.color.red).setDescription(`You already have ${arraylength} open tickets, as the current guild's ticket limit is ${ticketlimit} `).setAuthor(user.tag, user.displayAvatarURL()).setFooter('https://aquire.tk')).then(m => m.delete({timeout: 5000}));
