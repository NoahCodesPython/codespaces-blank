...
array.push(ID)
             ...
             message.channel.send(new discord.MessageEmbed().setColor(client.color.green).setTitle(language.ticketsetupReaction).setDescription(language.ticketsetupReactionCustomSuccess.replace("{option}", choice.toLowerCase())))
                .then(async () => {
                  let m = await channelToSend.send(new discord.MessageEmbed().setColor(client.color.green).setTitle(language.ticketTicketWord).setDescription(language.ticketHowToCreate).setFooter('https://syfer.aquire.tk'))
                   m.react("🎫");
