...
module.exports = class extends Command {
  ...
  async run(message, args) {
    ...
    const guildDB = await Guild.findOne({
      ...
      if(!db) return;
      if(db.ticketType !== "message") return message.channel.send(new Discord.MessageEmbed().setAuthor(`${message.author.tag}`, message.author.displayAvatarURL({ format: 'png' })).setDescription(`${message.client.emoji.fail} This Feature is disabled in the current guild`).setFooter('https://syfer.aquire.tk').setTimestamp().setColor('RED'));
      
      if(db.ticketToggle == "false") return message.channel.send(new Discord.MessageEmbed().setAuthor(`${message.author.tag}`, message.author.displayAvatarURL({ format: 'png' })).setDescription(`${message.client.emoji.fail} This Feature is disabled in the current guild`).setFooter('https://syfer.aquire.tk').setTimestamp().setColor('RED'));
  ...
  if (!reason) return message.channel.send(new Discord.MessageEmbed().setAuthor(`${message.author.tag}`, message.author.displayAvatarURL({ format: 'png' })).setDescription(`${message.client.emoji.fail} Please provide a reason`).setFooter('https://syfer.aquire.tk').setTimestamp().setColor('RED'));
  }
...
let arraylength = array.length
      ...
      if(arraylength > ticketlimit || arraylength == ticketlimit) {
        ...
        return message.channel.send(new discord.MessageEmbed().setColor(client.color.red).setDescription(`You already have ${arraylength} open tickets, as the current guild's ticket limit is ${ticketlimit} `).setAuthor(message.author.tag, message.author.displayAvatarURL()).setFooter('https://syfer.aquire.tk')).then(m => m.delete({timeout: 5000}))
      }
