...
let fail = message.client.emoji.fail
      let properUsage = new MessageEmbed()
        ...
        .setFooter('https://syfer.aquire.tk')
...
if(guildDB.suggestion.decline == "false"){
  return message.channel.send(new MessageEmbed().setAuthor(`${message.author.tag}`, message.author.displayAvatarURL({ format: 'png' })).setDescription(`${fail} Staff can't approve or decline Suggestions in this guild.`).setFooter('https://syfer.aquire.tk').setTimestamp().setColor('RED'));
}
if(!guildDB.suggestion.suggestionChannelID || !guildDB.suggestion.suggestionChannelID === null) return message.channel.send(new MessageEmbed().setAuthor(`${message.author.tag}`, message.author.displayAvatarURL({ format: 'png' })).setDescription(`${fail} ${language.suggesting1}`).setFooter('https://syfer.aquire.tk').setTimestamp().setColor('RED'));

let suggestion = guildDB.suggestion.suggestionChannelID
let channel = message.guild.channels.cache.get(suggestion)
if(!channel) return message.channel.send(new MessageEmbed().setAuthor(`${message.author.tag}`, message.author.displayAvatarURL({ format: 'png' })).setDescription(`${fail} ${language.suggesting2}`).setFooter('https://syfer.aquire.tk').setTimestamp().setColor('RED'));

if(!args[1]) return message.channel.send(new MessageEmbed().setAuthor(`${message.author.tag}`, message.author.displayAvatarURL({ format: 'png' })).setDescription(`${fail} ${language.suggesting12}`).setFooter('https://syfer.aquire.tk').setTimestamp().setColor('RED'));
...
var suggestionMsg = await channel.messages.fetch(args[1])
             ...
             message.channel.send(new MessageEmbed().setAuthor(`${message.author.tag}`, message.author.displayAvatarURL({ format: 'png' })).setDescription(`${fail} ${language.suggesting13}`).setFooter('https://syfer.aquire.tk').setTimestamp().setColor('RED'));
            return;
}
...
let description = suggestionMsg.embeds[0].description;
 ...
 if(suggestionMsg.embeds[0].title !== `${language.suggesting3}`) {
   ...
   if(suggestionMsg.embeds[0].title === `${language.suggesting14}`){
             message.channel.send(new MessageEmbed().setAuthor(`${message.author.tag}`, message.author.displayAvatarURL({ format: 'png' })).setDescription(`${fail} ${language.suggesting15}`).setFooter('https://syfer.aquire.tk').setTimestamp().setColor('RED'));
   } else {
             message.channel.send(new MessageEmbed().setAuthor(`${message.author.tag}`, message.author.displayAvatarURL({ format: 'png' })).setDescription(`${fail} ${language.suggesting16}`).setFooter('https://syfer.aquire.tk').setTimestamp().setColor('RED'));
   }
 ...
 if(!acceptReason) acceptReason = `${language.noReasonProvided}`;
   if(args.join(' ').length > 600) return message.channel.send(new MessageEmbed().setAuthor(`${message.author.tag}`, message.author.displayAvatarURL({ format: 'png' })).setDescription(`${fail} ${language.suggesting17}`).setFooter('https://syfer.aquire.tk').setTimestamp().setColor('RED'));
