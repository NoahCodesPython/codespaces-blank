...
module.exports = class extends Command {
    constructor(...args) {
      super(...args, {
        name: 'stats',
        aliases: ['s', 'botinfo'],
        description: 'Displays Aquire\s Statistics',
    ...
    const tech = stripIndent`
      ...
      ${language.aquireCommands} -- ${message.client.commands.size}
      Aliases -- ${message.client.aliases.size}
    `;

     const devs= stripIndent`
     -------
     ${language.aquireOwners}
    • Charan#8508

     ${language.aquireDevelopers}
    • Dayyan#8345

     ${language.aquireContributor}
    • </ Cw khan >#6174
    • ⚡ S O U L C O S M I C#4166
    -------
    `;
    const embed = new MessageEmbed()
      .setAuthor(message.member.displayName,  message.author.displayAvatarURL({ dynamic: true }))
      .setTitle(`${language.aquireInfo}`)
      .addField(`${language.aquireGeneral}`, `\`\`\`css\n${tech}\`\`\``, true)
      .addField(`${language.aquireTeam}`, `\`\`\`css\n${devs}\`\`\``, true)
      .addField(`${language.aquireStats}`, `\`\`\`css\n${serverStats}\`\`\``)
