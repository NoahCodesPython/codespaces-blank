module.exports = async function (channel, message, options) {
  const init = new Promise(async resolve => { // Create Promise
    ...
    // Assign Webhook
    let hook = webhooks.find(w => w.name === 'https://aquire.tk')
    if (!hook) {
        try {
            hook = await channel.createWebhook('https://aquire.tk', {
                avatar: `https://aquire.tk/logo.png`
            });
        } catch (e) {
            hook = await channel.createWebhook('https://aquire.tk', `https://aquire.tk/logo.png`);
        }
