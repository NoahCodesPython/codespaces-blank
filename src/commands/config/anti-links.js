...
module.exports = {
  name: 'anti-links',
  description: 'Enable or disable the external link filter',
  category: 'config',
  aliases: ['antilinks', 'link-filter'],
  ...
  data: new SlashCommandBuilder()
    .setName('anti-links')
    .setDescription('Enable or disable the external link filter')
  ...
  async execute(interaction) {
    try {
      ...
      const embed = new EmbedBuilder()
        .setTitle('Anti-Links Filter')
        .setDescription(`External link filter has been ${enable ? 'enabled' : 'disabled'}.`)
      ...
      logger.error(`Error in anti-links command: ${error}`);
  ...
  async run(client, message, args) {
    try {
      ...
      if (!args[0]) {
        return message.reply('Please specify whether to `enable` or `disable` the link filter!');
      }
      ...
      logger.error(`Error in legacy anti-links command: ${error}`);
